...
class TLRLS(Function):
    
    def __init__(self, theta_init: FloatArray2D, DF_discount: float, EF_discount: float):
        assert DF_discount > 0 and DF_discount <= 1, f"DF_discount must be in (0, 1]; DF_discount={DF_discount}"
        assert EF_discount > 0 and EF_discount <= 1, f"EF_discount must be in (0, 1]; EF_discount={EF_discount}"
        # DFの忘却係数
        self.DF_discount: float = DF_discount
        # EFの忘却係数
        self.EF_discount: float = EF_discount
        ...
        # 情報行列
        self.omega: FloatArray2D = np.zeros((theta_init.shape[0], theta_init.shape[0]), dtype=float)
        self.omega_a = self.omega

        # 情報行列の最小固有値の最大値
        self.__omega_eig_min = 0

        # 補助ベクトル
        self.M: FloatArray2D = np.zeros_like(self.theta)
        self.M_a = self.M

    def __calcN(self, phi: FloatArray2D) -> float:
        return np.sqrt(1 + phi.T @ phi)
    
    def __calcOmega(self, phi: FloatArray2D) -> None:
        
        n = self.__calcN(phi)

        # 情報行列のランクがフルでない場合は、phi @ phi.T/n**2 を加算してランクを上げる
        if np.linalg.matrix_rank(self.omega) == len(self.omega):
            if (np.linalg.matrix_rank(self.omega) < np.linalg.matrix_rank(self.omega + phi @ phi.T/n**2)):
                self.omega = self.omega + phi @ phi.T/n**2
            else:
                self.omega = self.EF_discount * self.omega - self.DF_discount * self.omega @ phi @ phi.T @ self.omega / (phi.T @ self.omega @ phi) + phi @ phi.T / n**2
        else:
            self.omega = self.omega + phi @ phi.T/n**2


    def __calcM(self, phi: FloatArray2D, y: float) -> None:
        """
        補助ベクトルの更新：M(k+1) = ...

        Args: 
            phi (FloatArray2D): リグレッサベクトル φ(k)．
            y (float): 観測出力 y(k)．
        """
        n = self.__calcN(phi)
        ...
        if np.linalg.matrix_rank(self.omega) == len(self.omega):
            if np.linalg.matrix_rank(self.omega) < np.linalg.matrix_rank(self.omega + phi @ phi.T/n**2):
                self.M = self.M + phi * y /n**2
            else:
                self.M = self.EF_discount * self.M - self.DF_discount * self.omega @ phi @ phi.T / (phi.T @ self.omega @ phi) @ self.M + phi * y / n**2
        else:
            self.M = self.M + phi * y /n**2
    ...
    def __calcTheta(self, phi: FloatArray2D, y: float) -> None:
        ...
        eta = 1
        self.theta = self.theta - eta*phi @ (self.theta.T @ phi - y)/self.__calcN(phi)**2 - eta*(self.omega_a @ self.theta - self.M_a)
    ...
    def forward(self, inputs):
        ...
        # 補助ベクトルの更新→情報行列の更新（順番注意）
        self.__calcM(phi, y)
        self.__calcOmega(phi)

        omega_eig_min = np.linalg.eigvalsh(self.omega)[0]
        if omega_eig_min > self.__omega_eig_min:
            self.__omega_eig_min = omega_eig_min

            # パラメータ更新に使う情報行列_aと補助ベクトル_aを更新
            self.omega_a = self.omega
            self.M_a = self.M
